...
var tourism = function (options) {
  return function (grunt) {
    ...
    grunt.registerTask('publish', function (releaseType) {
      ...
      grunt.task.run([ 'check-clean-repo', 'test', 'integrationtest', 'release:' + releaseType ]);
    });
    ...
    grunt.registerTask('check-repo', [ 'gitinfo', 'check-branch:master',
    'check-no-uncommitted-changes', 'shell:gitfetch', 'check-up-to-date' ]);
    ...
    // Helper tasks to let grunt continue even if an error occurs in some task
