...
var tourism = function (options) {
  return function (grunt) {
    ...
    grunt.registerTask('it', [ 'integrationtest' ]);
    ...
    // Note: stop-docker-XYZ fails if container is not running, so the force
    // option must be set for this task.
    grunt.registerTask('restart-docker', function(container) {
      var containers = [ container ];
      if (!container || container === 'all') {
        containers = [ 'mongo', 'rabbit' ];
      }

      containers.forEach(function (item) {
        grunt.task.run('set-force-option');
        grunt.task.run('shell:stop-docker-' + item);
        grunt.task.run('restore-force-option');
        grunt.task.run('shell:start-docker-' + item);
      });
    });
